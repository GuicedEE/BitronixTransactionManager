		...
		extends TestCase
{
	...
	protected void setUp() throws Exception
	{
		...
		XAPool<JdbcPooledConnection, JdbcPooledConnection> p1 = getPool(poolingDataSource1);
		registerPoolEventListener(p1);
		XAPool<JdbcPooledConnection, JdbcPooledConnection> p2 = getPool(poolingDataSource2);
		...
		while (iterator.hasNext())
		{
			...
			jdbcPooledConnection.addStateChangeEventListener(new StateChangeListener<JdbcPooledConnection>()
			{
				...
				public void stateChanged(JdbcPooledConnection source, State oldState, State newState)
				{
					...
					{
						EventRecorder.getEventRecorder(this)
						             .addEvent(new ConnectionQueuedEvent(this, source));
					}
					if (newState == State.ACCESSIBLE)
					{
						EventRecorder.getEventRecorder(this)
						             .addEvent(new ConnectionDequeuedEvent(this, source));
					}
				}
