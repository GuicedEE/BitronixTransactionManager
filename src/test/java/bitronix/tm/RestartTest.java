		...
		extends TestCase
{
	...
	public void testRestartWithLoader() throws Exception
	{
		for (int i = 0; i < 3; i++)
		{
			String configFile = new File(getClass().getResource("RestartTest.properties")
			                                       .toURI()).getPath();
			TransactionManagerServices.getConfiguration()
			                          .setResourceConfigurationFilename(configFile);
			BitronixTransactionManager tm = TransactionManagerServices.getTransactionManager();
			assertEquals("at loop iteration #" + (i + 1), 1, ResourceRegistrar.getResourcesUniqueNames()
			                                                                  .size());
			tm.shutdown();
			assertEquals("at loop iteration #" + (i + 1), 0, ResourceRegistrar.getResourcesUniqueNames()
			                                                                  .size());
		}
	}
