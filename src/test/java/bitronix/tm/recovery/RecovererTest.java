		...
		extends TestCase
{
	...
	{
		Iterator<String> it = ResourceRegistrar.getResourcesUniqueNames()
		                                       .iterator();
		while (it.hasNext())
		{
			String name = it.next();
	...
	{
		byte[] gtrid = UidGenerator.generateUid()
		                           .getArray();
		...
		assertEquals(0, TransactionManagerServices.getRecoverer()
		                                          .getCommittedCount());
		assertEquals(3, TransactionManagerServices.getRecoverer()
		                                          .getRolledbackCount());
	...
	public void testRecoverCommitting() throws Exception
	{
		...
		Xid xid1 = new MockXid(1, UidGenerator.generateUid()
		                                      .getArray(), BitronixXid.FORMAT_ID);
		xaResource.addInDoubtXid(xid1);
		Xid xid2 = new MockXid(2, UidGenerator.generateUid()
		                                      .getArray(), BitronixXid.FORMAT_ID);
	...
	public void testSkipInFlightRollback() throws Exception
	{
		...
		assertEquals(0, TransactionManagerServices.getRecoverer()
		                                          .getCommittedCount());
		assertEquals(1, TransactionManagerServices.getRecoverer()
		                                          .getRolledbackCount());
	...
	public void testRecoverMissingResource() throws Exception
	{
		...
		assertEquals(1, TransactionManagerServices.getJournal()
		                                          .collectDanglingRecords()
		                                          .size());
		assertNull(TransactionManagerServices.getRecoverer()
		...
		XAResource xaResource = handle.getPooledConnection()
		                              .getXAResource();
		connection.close();

		assertEquals(0, xaResource.recover(XAResource.TMSTARTRSCAN | XAResource.TMENDRSCAN).length);
		assertEquals(0, TransactionManagerServices.getJournal()
		                                          .collectDanglingRecords()
		                                          .size());
	...
	public void testBackgroundRecovererSkippingInFlightTransactions() throws Exception
	{
		...
		try
		{
			...
			xaResource.addInDoubtXid(new MockXid(new byte[]{0, 1, 2}, tx.getResourceManager()
			                                                            .getGtrid()
			                                                            .getArray(), BitronixXid.FORMAT_ID));
