		...
		implements StateChangeListener<JdbcPooledConnection>, JdbcPooledConnectionMBean
{
	...
	public JdbcPooledConnection(PoolingDataSource poolingDataSource, XAConnection xaConnection) throws SQLException
	{
		...
		statementsCache = new LruStatementCache(poolingDataSource.getPreparedStatementCacheSize());
		uncachedStatements = Collections.synchronizedList(new ArrayList<Statement>());
		...
		jmxName = "bitronix.tm:type=JDBC,UniqueName=" + ManagementRegistrar.makeValidName(poolingDataSource.getUniqueName()) + ",Id=" +
	...
	private void testConnection(Connection connection) throws SQLException
	{
		...
		if (poolingDataSource.isEnableJdbc4ConnectionTest() && jdbcVersionDetected >= 4)
		{
			...
			{
				log.warn("dysfunctional JDBC4 Connection.isValid() method, or negative acquisition timeout, in call to test connection of " + this +
