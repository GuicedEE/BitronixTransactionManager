...
public class Encoder
{
	...
	public static long bytesToLong(byte[] bytes, int pos)
	{
		if (bytes.length + pos < 8)
		{
			throw new IllegalArgumentException(
					"a long can only be decoded from 8 bytes of an array (got a " + bytes.length + " byte(s) array, must start at position " + pos + ")");
		}
	...
	public static int bytesToInt(byte[] bytes, int pos)
	{
		...
		{
			throw new IllegalArgumentException(
					"an integer can only be decoded from 4 bytes of an array (got a " + bytes.length + " byte(s) array, must start at position " + pos + ")");
		}
