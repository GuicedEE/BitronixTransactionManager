...
public class TransactionLogAppender
{
	...
	{
		this.file = file;
		randomeAccessFile = new RandomAccessFile(file, "rw");
		fc = randomeAccessFile.getChannel();
		header = new TransactionLogHeader(fc, maxFileLength);
		this.maxFileLength = maxFileLength;
		lock = fc.tryLock(0, TransactionLogHeader.TIMESTAMP_HEADER, false);
		if (lock == null)
		{
		...
		danglingRecords = new HashMap<Uid, Set<String>>();
	...
	protected List<TransactionLogRecord> getDanglingLogs()
	{
		synchronized (danglingRecords)
		{
			...
			Collections.sort(sortedUids, new Comparator<Uid>()
			{
				...
				{
					return Integer.valueOf(uid1.extractSequence())
